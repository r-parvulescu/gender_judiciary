...
def histogram_dict(iterable, idx, freq=bool):
    ...
    if isinstance(freq, int):
        return {k: len(g) for k, [*g] in itertools.groupby(iterable, lambda x: x[idx]) if len(g) <= freq}
    else:
        return {k: len(g) for k, [*g] in itertools.groupby(iterable, lambda x: x[idx])}
...
def string_tuple_by_ldist(strings, l_dist):
    ...
    and returns a list of pairs of strings ldist apart.
     """
    return sorted(list(filter(None, [(x, y) if Levenshtein.distance(x, y) == l_dist else ()
